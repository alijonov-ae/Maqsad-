...
# pip install -U python-telegram-bot Pillow qrcode[pil] gtts openai google-genai requests
# ============================================================
...
# Eslatma: alohida "WEBP Emoji" bo'limi olib tashlandi — endi rasm, video yoki GIF
# to'g'ridan-to'g'ri Premium Emoji oqimiga yuborilganda, bot ularni o'zi WEBP
# formatga (kerak bo'lsa animatsion) o'tkazib, emoji sifatida tayyorlaydi.
BASE_EMOJI_CHOICES = ["😀", "😂", "😍", "🔥", "👍", "❤️", "🎉", "😎", "🤩", "😢", "😡", "🤔"]

def _sanitize_pack_slug(title: str) -> str:
    """Foydalanuvchi kiritgan nomni Telegram sticker set name uchun xavfsiz slugga aylantiradi.
    Faqat a-z, 0-9 va _ qoldiriladi. Boshida harf bo'lishi shart.
    """
...
async def premium_emoji_start_new(update: Update, context: ContextTypes.DEFAULT_TYPE, back_keyboard, _reply_or_edit):
    ...
    context.user_data["pe_created"] = False
    context.user_data["pe_format"] = None
    context.user_data["pe_count"] = 0
...
async def premium_emoji_receive_title(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, BOT_USERNAME: str):
    ...
    # Foydalanuvchi kiritgan nom pack havolasiga (name) ham kiritiladi.
    # Nom har doim "_by_<bot_username>" bilan tugaydi — Telegram custom-emoji
    # to'plamlari shuni talab qiladi va bot shu tariqa to'plam egasi hisoblanadi.
    slug = _sanitize_pack_slug(pack_title)
    unique = f"{user_id}{int(datetime.now().timestamp()) % 100000}"
    bot_suffix = BOT_USERNAME.lstrip("@")
    pack_name = f"{slug}_{unique}_by_{bot_suffix}"
    if len(pack_name) > 64:
        # Telegram nomi 64 belgidan oshmasligi kerak
        max_slug = 64 - len(f"_{unique}_by_{bot_suffix}")
        pack_name = f"{slug[:max(1, max_slug)]}_{unique}_by_{bot_suffix}"
    ...
    await update.message.reply_text(
        ...
        f"Endi rasm (PNG/JPG), video yoki GIF yuboring — bot uni o'zi WEBP formatga o'tkazib emoji qiladi. "
        f"Birinchi faylni yuborishingiz bilan to'plam shu zahoti yaratiladi va sizga havola beriladi — "
        f"keyin xohlagancha qo'shishingiz mumkin.\n\n"
...
async def premium_emoji_select_pack(update: Update, context: ContextTypes.DEFAULT_TYPE, pack_name: str,
    ...
    context.user_data["pe_created"] = True  # Telegram'da allaqachon mavjud
    context.user_data["pe_format"] = None   # birinchi qo'shiladigan fayl formatidan aniqlanadi
    context.user_data["pe_count"] = pack[3] or 0
    context.user_data["state"] = "premium_emoji_collect"
    await _reply_or_edit(
        update,
        f"📦 <b>{html.escape(pack[2])}</b> pack'iga qo'shish\n\n"
        f"Rasm, video yoki GIF yuboring, har biriga mos emoji tanlaysiz. Tayyor bo'lgach \"✅ Yakunlash\" tugmasini bosing.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Yakunlash", callback_data="pe_finalize")],
            [InlineKeyboardButton("❌ Bekor qilish", callback_data="pe_cancel")],
        ])
...
def _is_animated_webp(webp_bytes: bytes) -> bool:
    """Yuborilgan .webp fayl animatsion (bir necha freym) yoki statikligini tekshiradi."""
    try:
...
async def premium_emoji_receive_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Foydalanuvchi oddiy rasm (photo) yuborganda ishlaydi — statik emoji sifatida tayyorlanadi."""
    ...
    context.user_data["pe_pending_format"] = "static"
...
async def premium_emoji_receive_webp_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Foydalanuvchi tayyor .webp fayl (statik yoki animatsion) yuborganda ishlaydi."""
    ...
    animated = _is_animated_webp(raw_bytes)
    try:
        if animated:
            # Allaqachon Telegram custom-emoji o'lchamida tayyorlangan — qayta ishlanmaydi
            webp_bytes = raw_bytes
        else:
            webp_bytes = create_emoji_from_image_bytes(raw_bytes)
    except Exception as e:
        logger.error(f"WEBP fayl tayyorlash xatosi: {e}")
        await update.message.reply_text("❌ WEBP faylni ishlashda xatolik yuz berdi, boshqa fayl yuboring.")
        return

    context.user_data["pe_pending_image"] = webp_bytes
    context.user_data["pe_pending_format"] = "animated" if animated else "static"
    context.user_data["state"] = "premium_emoji_choose_emoji"
    await update.message.reply_text(
        "😊 Bu fayl uchun mos asosiy emojini tanlang:",
        reply_markup=_emoji_choice_keyboard()
    )


async def premium_emoji_receive_video_or_animation(update: Update, context: ContextTypes.DEFAULT_TYPE, file_id: str):
    """Foydalanuvchi video yoki GIF (animatsiya) yuborganda ishlaydi — bot uni o'zi
    animatsion WEBP formatga o'tkazib, custom-emoji sifatida tayyorlaydi."""
    msg = await update.message.reply_text("⏳ Video/GIF WEBP formatga o'tkazilmoqda...")
    try:
        ...
        logger.error(f"Video/GIF'ni WEBP emoji uchun tayyorlash xatosi: {e}")
        await msg.edit_text("❌ Video/GIF'ni ishlashda xatolik yuz berdi, boshqa fayl yuboring.")
        return

    await msg.delete()
    context.user_data["pe_pending_image"] = webp_bytes
    context.user_data["pe_pending_format"] = "animated"
    context.user_data["state"] = "premium_emoji_choose_emoji"
    await update.message.reply_text(
        "😊 Bu fayl uchun mos asosiy emojini tanlang:",
        reply_markup=_emoji_choice_keyboard()
...
async def premium_emoji_receive_emoji_choice(update: Update, context: ContextTypes.DEFAULT_TYPE, emoji: str,
    ...
    pending_format = context.user_data.get("pe_pending_format", "static")

    if not pending:
        await query.answer("❌ Avval rasm yoki .webp fayl yuboring!", show_alert=True)
        return
    ...
    if pack_format and pack_format != pending_format:
        await query.answer(
            "❌ Bu to'plam boshqa formatda yaratilgan (statik/animatsion mos kelmadi). Boshqa fayl yuboring.",
            show_alert=True
        )
        return

    await query.answer("⏳ Qo'shilmoqda...")

    try:
        if not context.user_data.get("pe_created"):
            # Birinchi element — to'plam shu zahoti yaratiladi va havola beriladi
            stickers_input = [InputSticker(sticker=pending, emoji_list=[emoji], format=pending_format)]
            ...
            context.user_data["pe_created"] = True
            context.user_data["pe_format"] = pending_format
        else:
            await context.bot.add_sticker_to_set(
                user_id=user_id,
                name=pack_name,
                sticker=InputSticker(sticker=pending, emoji_list=[emoji], format=pending_format),
            )
            if not context.user_data.get("pe_format"):
                context.user_data["pe_format"] = pending_format

        increment_pack_item_count(pack_name, 1, DB_PATH)
        increment_usage(user_id, "premium_emoji")
        context.user_data["pe_count"] = context.user_data.get("pe_count", 0) + 1
        count = context.user_data["pe_count"]
        link = f"https://t.me/addemoji/{pack_name}"

        out = io.BytesIO(pending)
        out.name = "emoji.webp"
        ...
        await query.message.reply_document(
            out,
            caption=(
                f"✅ Emoji qo'shildi! ({emoji})\n\n"
                f"📦 Jami: {count} ta\n🔗 Havola: {link}\n\n"
                f"Yana fayl yuboring yoki yakunlang:{BOT_FOOTER}"
            ),
            reply_markup=kb
        )
    except Exception as e:
        logger.error(f"Premium emoji qo'shishda xatolik: {e}")
        await query.message.reply_text(f"❌ Qo'shishda xatolik yuz berdi: {e}")
...
async def premium_emoji_finalize(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    query = update.callback_query
    pack_name = context.user_data.get("pe_pack_name")
    count = context.user_data.get("pe_count", 0)

    if not context.user_data.get("pe_created") or not pack_name:
        await query.message.reply_text("📭 Hali hech qanday emoji qo'shilmadi! Avval kamida bitta rasm yoki .webp fayl yuboring.")
        return

    link = f"https://t.me/addemoji/{pack_name}"
    await query.message.reply_text(
        f"✅ Premium emoji to'plam tayyor!\n\n📦 Jami: {count} ta emoji\n🔗 Havola: {link}\n\n"
        f"Havolani bosib, emojilarni Telegramga qo'shib oling!{BOT_FOOTER}",
    )
    ...
    context.user_data.pop("pe_pending_format", None)
    context.user_data.pop("pe_is_new_pack", None)
    context.user_data.pop("pe_created", None)
    context.user_data.pop("pe_format", None)
    context.user_data.pop("pe_count", None)
    context.user_data["state"] = None
    await increment_action_and_check_ad(update, context)
...
async def ask_universal_ai(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    # Avval sozlangan Gemini (Google AI) orqali javob olishga harakat qilamiz —
    # GEMINI_API_KEY muhit o'zgaruvchisi berilgan bo'lsa, gemini_client tayyor bo'ladi.
    if gemini_client is not None:
        try:
            ...
            answer = (getattr(response, "text", None) or "").strip() or None
        except Exception as e:
            logger.error(f"Gemini AI xato: {e}")
            answer = None

    # Gemini ishlamasa (yoki kaliti yo'q bo'lsa), zaxira sifatida Bytez orqali
    # boshqa provayderlarni sinab ko'ramiz.
    if not answer:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_sub":
        ...
        await premium_emoji_receive_emoji_choice(update, context, emoji, DB_PATH, check_limit, increment_usage, increment_action_and_check_ad)
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "circle_video_input" and (update.message.video or update.message.video_note):
        ...
        await premium_emoji_receive_video_or_animation(update, context, video_obj.file_id)
...
async def handle_other_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "premium_emoji_collect" and update.message.document:
        doc = update.message.document
        fname = (doc.file_name or "").lower()
        mime = (doc.mime_type or "").lower()
        if fname.endswith(".webp") or mime == "image/webp":
            await premium_emoji_receive_webp_document(update, context)
        else:
            await update.message.reply_text("❌ Faqat rasm yoki .webp fayl yuboring.")
        return
    ...
    if state == "premium_emoji_collect" and update.message.animation:
        await premium_emoji_receive_video_or_animation(update, context, update.message.animation.file_id)
        return
