...
async def handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    last_at = get_setting("last_reaction_at", "")
    if last_at:
        try:
    ...
    try:
        ...
        set_setting("last_reaction_at", datetime.now().isoformat())
    except Exception as e:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif state == "set_reaction_channel" and user_id in ADMIN_IDS:
        channel = text.strip()
        if not channel.lstrip("-").isdigit() and not channel.startswith("@"):
            channel = "@" + channel
        set_setting("reaction_channel", channel)
        context.user_data["state"] = None
        await update.message.reply_text(
            f"✅ Avto-reaksiya kanali o'rnatildi: {channel}\n\n"
            f"⚠️ Botni shu kanalga <b>admin</b> qilib qo'shishni unutmang!",
            parse_mode="HTML",
