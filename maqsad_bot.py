...
async def send_goal_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Bajardim", callback_data=f"maqsad_done_{goal_id}")]
    ])
    text = (
        f"🔔 <b>{html.escape(name)}</b>\n\n"
        f"Maqsadni bajarish kerak! Men eslatdim, seni vijdoningga havola.\n\n"
        f"📊 Eslatma: {sent_count}/{target_count}"
    )
    try:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎯 Maqsad":
        ...
        await update.message.reply_text("🔢 Necha marta eslatma yuborilsin? (1-100 oralig'ida son kiriting):")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("maqsad_done_"):
        goal_id = int(data.replace("maqsad_done_", ""))
        deactivate_goal(goal_id)
        for job in context.job_queue.get_jobs_by_name(f"goal_{goal_id}"):
            job.schedule_removal()
        await query.edit_message_text("🎉 Ajoyib! Maqsad bajarildi deb belgilandi. Tabriklaymiz!")
