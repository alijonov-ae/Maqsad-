# ============================================================
# MAQSAD BOT — To'liq original kodi + AI Kamera Webhook API
#
# Kerakli kutubxonalar:
#   pip install "python-telegram-bot[job-queue]==20.7" fastapi uvicorn pydantic
#
...
#   WEBHOOK_SECRET=maqsad_sec_key_2026_super_secret  (Sayt bilan ulanish uchun)
# ============================================================
...
# Global Telegram Application Reference (Webhook uchun)
telegram_app = None

# ── DATABASE INIZIALIZATSIYASI ───────────────────────────────
def init_db():
    conn = sqlite3.connect("bot.db", timeout=15)
    ...
    # Agar eski bazada "goals" jadvali done_count ustunisiz allaqachon mavjud bo'lsa,
    # uni xavfsiz qo'shib qo'yamiz (yangi bazalarda bu blok hech narsa qilmaydi).
    c.execute("PRAGMA table_info(goals)")
    goal_columns = [row[1] for row in c.fetchall()]
    if "done_count" not in goal_columns:
        c.execute("ALTER TABLE goals ADD COLUMN done_count INTEGER DEFAULT 0")
...
def get_all_active_goals_for_user(user_id):
    conn = sqlite3.connect("bot.db", timeout=15)
    conn.execute("PRAGMA journal_mode=WAL;")
    c = conn.cursor()
    c.execute("""SELECT id, name, target_count, done_count, interval_minutes
                 FROM goals WHERE user_id=? AND is_active=1""", (user_id,))
    rows = c.fetchall()
    conn.close()
    return rows

def increment_goal_done(goal_id, increment=1):
    conn = sqlite3.connect("bot.db", timeout=15)
    conn.execute("PRAGMA journal_mode=WAL;")
    c = conn.cursor()
    c.execute("UPDATE goals SET done_count = done_count + ? WHERE id=?", (increment, goal_id))
    conn.commit()
    c.execute("SELECT done_count, target_count, name FROM goals WHERE id=?", (goal_id,))
    row = c.fetchone()
    conn.close()
    return row
...
def main_menu_keyboard():
    keyboard = [
        [KeyboardButton("🎯 Maqsad"), KeyboardButton("📊 Holatim")],
        [KeyboardButton("🔗 AI Kamera Veb Ilovasiga Bog'lanish")]
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (
        ...
        f"Maqsad qoʻyish uchun /maqsad buyrugʻini yuboring yoki pastdagi '🎯 Maqsad' tugmasini bosing.\n"
        f"AI Kamera Veb ilovasiga ulanish uchun '🔗 AI Kamera Veb Ilovasiga Bog'lanish' tugmasidan foydalaning!"
    )
...
# ── HOLATIM BO'LIMI ────────────────────────────────────────────
async def holatim_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    goals = get_all_active_goals_for_user(user_id)

    if not goals:
        await update.message.reply_text("Sizda hozircha faol maqsadlar yo'q. '🎯 Maqsad' tugmasi orqali qo'shing!")
        return

    msg = "<b>📊 Sizning Faol Maqsadlaringiz:</b>\n\n"
    keyboard = []
    for g_id, g_name, g_target, g_done, g_interval in goals:
        msg += f"🎯 <b>{html.escape(g_name)}</b>\n"
        msg += f"   • Bajarildi: {g_done}/{g_target} ta\n"
        msg += f"   • Eslatma: Har {g_interval} daqiqada\n\n"
        keyboard.append([InlineKeyboardButton(f"❌ Bekor qilish: {g_name[:15]}", callback_data=f"maqsad_cancel_{g_id}")])
...
async def send_goal_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"📸 Bajarganingizga isbot sifatida <b>rasm yoki video</b> yuboring yoki AI Kamera Veb ilovasidan foydalaning!\n"
...
async def process_goal_proof(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    res = increment_goal_done(goal_id, 1)
    new_done = res[0] if res else done_count + 1
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Har doim menyu tugmalari bosilganda eski state'ni tozalash
    if text in ["🎯 Maqsad", "📊 Holatim", "🔗 AI Kamera Veb Ilovasiga Bog'lanish"]:
    ...
    if text == "🎯 Maqsad":
        ...
        await holatim_command(update, context)
        return
    elif text == "🔗 AI Kamera Veb Ilovasiga Bog'lanish":
        await update.message.reply_html(
            f"<b>🔗 AI Kamera Veb Ilovasiga Bog'lanish Ma'lumotlari:</b>\n\n"
            f"Sizning Telegram ID ingiz: <code>{user_id}</code>\n"
            f"Secret API Key: <code>{WEBHOOK_SECRET}</code>\n\n"
            f"Veb ilovada shu ma'lumotlarni kiriting va jonli AI Kamera orqali mashqlaringizni bajaring!"
        )
        return
        ...
        context.job_queue.run_repeating(
            send_goal_reminder,
            interval=interval * 60,   # Fix interval bug
            first=interval * 60,
        ...
        await update.message.reply_text(
            ...
            f"🔢 Tasdiqlash soni: {count} marta\n"
            f"⏰ Eslatma: Har {interval} daqiqada.\n\n"
            f"📸 Bajarganingizga isbot sifatida rasm/video yuboring yoki Veb AI Kamera ilovasidan foydalaning!",
            parse_mode="HTML"
        )
...
# ── FASTAPI WEBHOOK INTEGRATSIYASI (VEB ILOWA BILAN ALOQA) ───
app = FastAPI(title="Maqsad Bot AI Camera Webhook Bridge")
...
async def handle_goal_progress_webhook(
    payload: WebhookProgressPayload,
    x_bot_api_key: Optional[str] = Header(None)
):
    key = x_bot_api_key or payload.api_key
    if key != WEBHOOK_SECRET:
        raise HTTPException(status_code=401, detail="Xavfsizlik kaliti xato!")

    try:
        raw_id = str(payload.goal_id).replace("goal_", "")
        goal_id_int = int(raw_id)
    except ValueError:
        goal = get_active_goal_for_chat(payload.telegram_id)
        if not goal:
            return {"success": False, "error": "Faol maqsad topilmadi"}
        goal_id_int = goal[0]

    res = increment_goal_done(goal_id_int, payload.increment)
    if res:
        done, target, name = res
        is_completed = done >= target

        if is_completed:
            deactivate_goal(goal_id_int)

        message = (
            f"🎉 <b>TABRIKLAYMIZ!</b>\n'{html.escape(name)}' maqsadingiz to'liq bajarildi! ({done}/{target} ✅)"
            if is_completed else
            f"💪 <b>+1 Push-Up bajarildi!</b>\n'<b>{html.escape(name)}</b>': {done}/{target} ta"
        )
    ...
    return {"success": False, "error": "Maqsad topilmadi"}

# ── HYBRID HYBRID RUNNER (FASTAPI + PTB) ──────────────────────
async def run_hybrid_bot():
    ...
    for goal_id, chat_id, interval_minutes in active_goals:
        interval_val = interval_minutes if interval_minutes else 60
        telegram_app.job_queue.run_repeating(
            send_goal_reminder,
            interval=interval_val * 60,
            first=interval_val * 60,
            data={"goal_id": goal_id},
            name=f"goal_{goal_id}",
            chat_id=chat_id
        )
    ...
    telegram_app.add_handler(CommandHandler("maqsad", maqsad_command))
    telegram_app.add_handler(CommandHandler("holatim", holatim_command))
    telegram_app.add_handler(CommandHandler("Premiumoldi", premium_oldi))

    telegram_app.add_handler(CallbackQueryHandler(callback_handler))

    telegram_app.add_handler(MessageHandler(filters.PHOTO, handle_photo))
    telegram_app.add_handler(MessageHandler(filters.VIDEO | filters.VIDEO_NOTE, handle_video))
    telegram_app.add_handler(MessageHandler(filters.ANIMATION | filters.AUDIO | filters.VOICE | filters.Document.ALL | filters.Sticker.ALL, handle_other_media))
    ...
    print("✅ Maqsad Bot & FastAPI Webhook birga ishga tushdi!")
    await server.serve()
